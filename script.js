...
const invoiceNumberDisplay = document.getElementById('invoice-number-display'); // Added
...
generateBillButton.addEventListener('click', () => {
  ...
  const whatsappLink = `https://wa.me/${customerNumber}?text=${encodeURIComponent(billMessage)}`;
  window.open(whatsappLink, '_blank');

  // Clear fields and scanned products
  customerNameInput.value = '';
  customerNumberInput.value = '';
  scannedProducts = [];
  updateDisplay();
  invoiceNumberDisplay.textContent = invoiceNumber; //display invoice number
  invoiceCounter++;
});
...
function generateBillMessage(customerName, invoiceNumber) {
  ...
  //billMessage += `Invoice Number: ${invoiceNumber}\n\n`;
  if (customerName) {
